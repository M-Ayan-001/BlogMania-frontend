...
const MyLikedblogs = () => {
    ...
    useEffect(() => {
        axios.get(`${API}/blog/mylikedblogs/${id}`, { withCredentials: true })
            .then((res) => {
                ...
                <Navbar />
                {
                    loading ? (
                        ...
                        <div className='w-full mt-25 flex flex-wrap'>
                            {
                                blogs.length === 0 ? (
                                    <h1 className='text-2xl font-bold tracking-tighter w-full text-center'>You have not liked any blog</h1>
                                ) : (
                                    blogs.map((blog) =>
                                    (
